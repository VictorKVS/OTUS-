...
- **Подтверждение:** `README_DZ9.md`, `CLOUD_VS_ONPREM.md`, `llm_tco_calculator.py`, `SOURCES.md`
...
- быстрый старт без собственного GPU CAPEX.
...
- возможен vendor lock-in на API и поведение конкретного провайдера.
...
- при низкой утилизации GPU экономическое преимущество on-prem может исчезнуть.
...
|---|---|---|---|
| **Стоимость** | Нет собственного GPU CAPEX, но переменные расходы растут вместе с количеством запросов/токенов | Требуются собственные ресурсы, амортизация, электричество и эксплуатация; зато нет внешней per-token тарификации основного инференса | Нужен TCO-расчёт; при существующем локальном контуре Self-hosted даёт более контролируемые расходы |
...
| **Качество** | Frontier SaaS обычно является сильным вариантом для сложных reasoning-задач | Качество зависит от выбранной open-source модели, квантизации и оборудования | **Преимущество SaaS**, поэтому нужен общий quality gate |
| **Latency** | Есть сетевой RTT; backend провайдера обычно хорошо масштабирован | Нет внешнего сетевого RTT, но скорость зависит от локального GPU и размера модели | Для локальной нагрузки Self-hosted удобен; при высокой параллельности преимущество может измениться |
...
### 5.1 Воспроизводимая TCO-модель

Для Hosted API используется:
...
Отдельно учитывается полезная стоимость GPU-часа:

`Effective GPU-hour = total_monthly_cost / (available_hours × utilization)`

Подробная модель находится в `CLOUD_VS_ONPREM.md`, а воспроизводимый расчёт — в `llm_tco_calculator.py`.

### 5.2 Демонстрационный сценарий

Текущая версия калькулятора использует учебные предпосылки:

- API: 100 000 запросов/месяц;
- 2 000 input и 500 output tokens на запрос;
- условные rates: 2 и 8 денежных единиц за 1M tokens;
- on-prem CAPEX: 5 000;
- амортизация: 36 месяцев;
- 0.45 kW, 730 часов/месяц, 0.15/kWh, PUE 1.2;
- operations & maintenance: 150/месяц;
- utilization: 60%.
...
Эти числа **не являются текущим коммерческим прайсом** и не доказывают, что self-hosted всегда дешевле. Они демонстрируют воспроизводимую методику. Перед реальным решением должны быть актуализированы тарифы API, CAPEX, стоимость электроэнергии, utilization, SLA и эксплуатационные трудозатраты.

### 5.3 Quality Gate

Стоимость сравнивается только при сопоставимом качестве и SLO. Self-hosted нельзя признать победителем только потому, что сценарный TCO ниже, если локальная модель не проходит тот же evaluation set, что SaaS.

Минимально сравниваются:

- качество ответа на общем eval-наборе;
- faithfulness / answer relevancy для RAG;
- p50/p95 latency;
- throughput и параллельность;
- ошибки и очереди;
- фактические эксплуатационные затраты.
...
3. **Воспроизводимый TCO-подход.** Решение можно пересчитывать при изменении нагрузки и тарифов вместо использования субъективного мнения.
...
7. **Риск низкой утилизации.** При простое GPU эффективная стоимость полезного GPU-часа растёт.
...
| Низкая утилизация GPU | Рост effective GPU-hour и плохой TCO | мониторинг utilization, пересчёт capacity, пересмотр ADR |
...
ADR переводится в `Superseded` или заменяется новым ADR, если выполняется хотя бы одно условие:
...
История решения не переписывается: новый подход должен supersede-ить этот ADR.

## 10. CTO Pitch

**Предлагаю зафиксировать Self-hosted LLM как основной inference-контур текущего проекта, потому что это решение лучше всего соответствует нашим текущим архитектурным драйверам: уже построенному локальному стеку, контролю данных, воспроизводимости модели и управляемой зависимости от внешних сервисов. Мы не утверждаем, что локальная модель всегда дешевле или качественнее SaaS: экономику проверяем воспроизводимой TCO-моделью, а качество — единым evaluation set и SLO. Мы сознательно принимаем более высокую операционную сложность и возможный разрыв в качестве с frontier SaaS; если измерения покажут, что локальный контур перестал удовлетворять качеству, SLA или TCO, решение будет superseded новым ADR. В текущих условиях Self-hosted является наиболее обоснованным и проверяемым вариантом.**
...
Может получиться. Self-hosted переносит стоимость из API usage в оборудование, электроэнергию и эксплуатацию. Поэтому ADR не утверждает, что локальный inference всегда дешевле; он требует сценарного TCO и последующего пересчёта на фактических данных.

### Почему демонстрационный расчёт нельзя считать доказательством?

Потому что rates, CAPEX и эксплуатационные параметры в нём учебные. Его назначение — показать воспроизводимую формулу и набор измеряемых величин. Финальное экономическое решение должно опираться на актуальные тарифы и телеметрию.
...
7. воспроизводимый запуск `llm_tco_calculator.py`;
8. перечень рисков и условия пересмотра ADR.

## 12. Источники

Полный перечень вынесен в `SOURCES.md`.

Использованы:

- общие принципы ADR и неизменяемой истории решений;
- MADR как ориентир структуры;
- материал a16z о trade-offs hosted/cloud/on-prem AI compute;
- каталог ADR tooling для последующей автоматизации.

Исторические числовые сведения из внешних материалов не используются как актуальный прайс без повторной проверки.

## 13. Итог

Решение **Accepted**: основной inference-контур — **Self-hosted Open-Source LLM**. SaaS сохраняется как возможная будущая альтернатива/дополнительный backend, но его использование в качестве основного варианта требует нового ADR, который supersede-ит это решение.
